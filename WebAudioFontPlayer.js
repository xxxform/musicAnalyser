...
function WebAudioFontLoader(player) {
	...
	this.startLoad = function(audioContext, filePath, variableName) {
		...
		//Вот тут грузятся из интернета библиотеки звуков
...
function WebAudioFontPlayer() {
	...
	this.createReverberator = function (audioContext) {
		this.startBuffer = audioContext.createBuffer(1, 1, 44100);
	...
	this.queueWaveTable = function (audioContext, target, preset, when, pitch, duration, volume, slides, callbackEnd, callbackStart) {
		...
		// зоны в window попадают из файлов с настройками sf2 где они объявляются как var
		// зоны это диапазоны клавиатуры за которыми закрепляется определённое звучание семпла. 
		// originalPitch - частота семпла sf2 зоны которая изменяется в пределах от keyRangeLow до keyRangeHigh простым транспонированием
		// содержит 2 лишних нуля
		...
		var startWhen = when;
		if (startWhen < audioContext.currentTime) {//Выставляется когда тормоза
		...
		//Создан буфер, который хранит семпл звука, что будет воспроизведён 
		//т.е послан на выход audioContext.destination(колонки)
		envelope.audioBufferSourceNode = audioContext.createBufferSource();
		envelope.audioBufferSourceNode.playbackRate.setValueAtTime(playbackRate, 0);
		
		//Этот буфер можно изменять во время воспроизведения
		
		if (slides && slides.length > 0) {
			if (slides[0].when === 0) envelope.audioBufferSourceNode.detune.value = slides[0].pitch;
			for (let i = 0; i < slides.length; i++)
				envelope.audioBufferSourceNode.detune.setValueAtTime(slides[i].pitch, when + slides[i].when);
		}
		...
		if (typeof callbackEnd === 'function') envelope.audioBufferSourceNode.onended = callbackEnd;
		
		envelope.audioBufferSourceNode.start(startWhen, zone.delay);
		envelope.audioBufferSourceNode.stop(startWhen + waveDuration);

		if (typeof callbackStart === 'function') {
			const startBuffer = audioContext.createConstantSource();
			startBuffer.buffer = this.startBuffer;
			startBuffer.connect(envelope); //Влияет ли disconnect и можно ли без него?
			startBuffer.onended = () => {callbackStart(envelope.audioBufferSourceNode); startBuffer.disconnect(envelope)}
			startBuffer.start(startWhen, zone.delay);
			startBuffer.stop(startWhen);
		}
