...
MIDIFile.prototype.addSlide = function (event, song) {//Запускается когда парсер натыкается на событие колеса
	var track = this.takeTrack(event.channel, song); //Создаст трек или вернёт существующий
	
	for (var i = 0; i < track.notes.length; i++) { // Ищет звуки
		if (track.notes[i].duration == 0.0000001 //не закрытые в данный момент парсинга
			 && track.notes[i].when < event.playTime / 1000) {//По времени раньше слайда т.е слайд всегда позже
			//event.param1 это число после запятой числа param2 для более точной передачи высоты. макс 127
			track.notes[i].slides.push({ //event param2 событие колеса. min 0 64 по умолчанию max 127(2тона по стандарту). 
				//								Целая часть				дробная часть      множитель для приведения к двойке
				pitch: Math.round((((event.param2 - 64) + (1 / 128) * event.param1) * (2 / 64)) * 100), //(event.param2 - 64) = 64 колесо до упора вверх, -64 вниз
				when: event.playTime / 1000 - track.notes[i].when// 2/64 мельчайшее значение
				//Проблема. Макс 127 а не 128 поэтому слайд макс вверх всегда немного ниже чем 2
			});
			//рассчет без дробной части: pitch: track.notes[i].pitch + (event.param2 - 64) * (2 / (event.param2 > 64 ? 63: 64))
		}
	}
}
...
MIDIFile.prototype.parseSong = function () {
	...
	const trackLastSlideMap = {};
	
	const push = e => song.meta.push(e);
	
	const handlers = {
		[MIDIEvents.EVENT_META_TIME_SIGNATURE]: push,
		[MIDIEvents.EVENT_META_KEY_SIGNATURE]: push,
		[MIDIEvents.EVENT_META_SET_TEMPO]: push,
		[MIDIEvents.EVENT_MIDI_NOTE_ON]: e => {
			if (e.channel == 9) { 
				if (e.param1 >= 35 && e.param1 <= 81) {
					this.startDrum(e, song);
				} else {
					console.log('wrong drum', e);
				}
			} else {
				if (e.param1 >= 0 && e.param1 <= 127) {
					const note = this.startNote(e, song);
					
					const lastSlide = trackLastSlideMap[e.channel];     //Колесо не в равновесии
					if (note.slides.length === 0 && lastSlide !== void 0 && lastSlide.param2 !== 64) {
						note.slides.push({ 
							pitch: Math.round((((lastSlide.param2 - 64) + (1 / 128) * lastSlide.param1) * (2 / 64)) * 100), //Все вычисления лучше сделать при парсинге
							when: 0 //Время от начала звука
						});
					}
				} else {
					console.log('wrong tone', e);
				}
			}
		},
		[MIDIEvents.EVENT_MIDI_NOTE_OFF]: e => {
			if (e.channel != 9) { // Завершает все звуки кроме шумовых
				this.closeNote(e, song); 
			}
		},
		[MIDIEvents.EVENT_MIDI_PROGRAM_CHANGE]: e => {
			if (e.channel != 9) {
				var track = this.takeTrack(e.channel, song);
				track.program = e.param1;
			} else {
				console.log('skip program for drums');
			}
		},
		[MIDIEvents.EVENT_MIDI_CONTROLLER]: e => {
			if (e.param1 == 7) {
				if (e.channel != 9) {
					var track = this.takeTrack(e.channel, song);
		...
		[MIDIEvents.EVENT_MIDI_PITCH_BEND]: e => {
			trackLastSlideMap[e.channel] = e;
			this.addSlide(e, song); //Ищет открытые ноты канала
		}
	};
	
	//Записывать надо карту канал - последний слайд и перед startNote добавлять если он не 0
	//Для шумовых слайд невозможен тк в addslide перебирается song.track

	var events = this.getEvents(
		[MIDIEvents.EVENT_MIDI, MIDIEvents.EVENT_META], 
		Object.keys(handlers).map(Number)
	);
	...
	for (var i = 0; i < events.length; i++) {
		...
		if (typeof handlers[events[i].subtype] == 'function') 
			handlers[events[i].subtype](events[i]);
		else 
...
MIDIFile.prototype.getEvents = function (type, subtype) {
	var events;
	var event;
	var playTime = 0; //Именно в этой функции устанавливается. Часто завязан на e.delta
	var filteredEvents = [];
		...
		for (i = 0, j = this.tracks.length; i < j; i++) {
			...
			while (event) {
				...
				if (((!type) || type.includes(event.type) &&
					((!subtype) || (event.subtype && subtype.includes(event.subtype))))) {
